...
extension Dictionary {
  ...
  @inlinable public func mapKeysAsync<K>(
    _ transform: @Sendable @escaping (Key) async throws -> K
  ) async rethrows -> [K: Value] where K: Hashable & Sendable, Key: Sendable, Value: Sendable {
    return try await withThrowingTaskGroup(of: (K, Value).self) { group in
      for (key, value) in self { group.addTask { (try await transform(key), value) } }
      return try await group.reduce(into: [K: Value]()) { result, pair in result[pair.0] = pair.1 }
    }
  }
  ...
  @inlinable public func mapKeysAndValuesAsync<K, V>(
    _ transform: @Sendable @escaping (Key, Value) async throws -> (K, V)
  ) async rethrows -> [K: V]
  where K: Hashable & Sendable, V: Sendable, Key: Sendable, Value: Sendable {
    return try await withThrowingTaskGroup(of: (K, V).self) { group in
      for (key, value) in self { group.addTask { try await transform(key, value) } }
      return try await group.reduce(into: [K: V]()) { result, pair in result[pair.0] = pair.1 }
    }
  }
  ...
  @inlinable public func mapValuesAsync<V>(
    _ transform: @Sendable @escaping (Value) async throws -> V
  ) async rethrows -> [Key: V] where V: Sendable, Key: Sendable, Value: Sendable {
    return try await withThrowingTaskGroup(of: (Key, V).self) { group in
      for (key, value) in self { group.addTask { (key, try await transform(value)) } }
      return try await group.reduce(into: [Key: V]()) { result, pair in result[pair.0] = pair.1 }
    }
  }
}
