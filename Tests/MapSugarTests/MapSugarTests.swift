...
@Test func mapValuesAsyncRethrows() async {
  enum TestError: Error { case bad }

  await #expect(throws: TestError.bad) {
    _ = try await ["a": 1].mapValuesAsync { _ -> String in throw TestError.bad }
  }
}
...
@Test func mapValuesUsingKeysAsyncRethrows() async {
  enum TestError: Error { case bad }

  await #expect(throws: TestError.bad) {
    _ = try await ["a": 1].mapValuesUsingKeysAsync { _, _ -> String in throw TestError.bad }
  }
}
...
@Test func mapKeysAndValuesAsyncRethrows() async {
  enum TestError: Error { case bad }

  await #expect(throws: TestError.bad) {
    _ = try await ["a": "x"]
      .mapKeysAndValuesAsync { _, _ -> (String, String) in throw TestError.bad }
  }
}
